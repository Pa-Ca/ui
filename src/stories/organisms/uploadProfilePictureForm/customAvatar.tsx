...
const CustomAvatar = ({
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    onBeforeFileLoad && onBeforeFileLoad(event);
    ...
    if (event.target.files && event.target.files[0]) {
      ...
      reader.onload = (e) => {
        if (e.target && e.target.result) {
          toDataURL(e.target.result as string)
          .then(dataUrl  => {
  ...
  const handleMouseDown = (event: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    const distanceFromCenter = Math.sqrt((x - circleX) ** 2 + (y - circleY) ** 2);
    ...
    if (distanceFromCenter >= radius * 0.75 && distanceFromCenter <= radius * 1.25) {
  ...
  const handleMouseUp = () => {
    ...
    // Create preview
    let imageData = ctx.getImageData(circleX - radius, circleY - radius, radius * 2, radius * 2);
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      ...
      const imageData = ctx.getImageData(circleX - mod_radius, circleY - mod_radius, mod_radius * 2, mod_radius * 2);
  ...
  }, [image, circleX, circleY, radius, updateCanvas]);
  ...
  return (
    <div>
      ...
      <button onClick={onClose} style={{ display: 'none' }}>Close</button>
      {image && (
        <div>
          <canvas
            ref={canvasRef}
