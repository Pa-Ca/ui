...
export const EditableInputTime = ({
  ...
  const formatTime = (time: InputFormHook<string>) => {
    if (!time.value || time.value.length === 0) time.setValue("00");
    else if (time.value.length === 1) time.setValue("0" + time.value);
